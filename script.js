...
// BoostボタンがHTMLにない場合に動的に追加
function ensureBoostButton() {
    if (!document.getElementById("boost-btn")) {
        const actionArea = document.querySelector(".action-area");
        const btn = document.createElement("button");
        btn.id = "boost-btn";
        btn.className = "btn-base boost-btn"; // CSSで .boost-btn のスタイルを当ててください
        btn.onclick = buyBoost;
        btn.innerHTML = `<span class="btn-label">STAR BOOST [R]</span><small id="boost-info">Cost: 1.00e5</small>`;
...
function updateUI(gain) {
    ...
    document.getElementById("overdrive-text").style.display = overdrive ? "block" : "none";
...
function saveGame(show) {
    ...
    localStorage.setItem("star_enhanced_save", JSON.stringify(data));
    if(show) { let p=document.getElementById("save-popup"); p.style.opacity=1; setTimeout(()=>p.style.opacity=0, 1000); }
}

function loadGame() {
    let s = localStorage.getItem("star_enhanced_save");
    if (!s) return;
    let d = JSON.parse(s);
